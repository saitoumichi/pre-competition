...
# =========================================================
# Utility: threshold selection (Accuracy maximize)
# =========================================================
...
class WarmupCosineScheduler(torch.optim.lr_scheduler._LRScheduler):
    ...
    def get_lr(self):
        ...
        progress = (self.last_epoch - self.warmup_epochs) / denom
...
def main():
    ...
    # ★ 1ch 用 normalize（平均/分散は安全側の仮値）
    # もしデータの平均・分散を計算して合わせたいなら後で置換可能
    ...
    # TensorBoard（rank0だけ）
    base_log_dir = r"D:\puresotu\workespace\nakayama_ken-main\nakayama_ken-main\result_kazma\runs_effb4_acc_no_kfold_1ch"
...
model=tf_efficientnet_b4_ns (in_chans=1)
...
amp={use_amp}
mixup_linear_p={mix_p}
mix_alpha={mix_alpha}
scheduler=WarmupCosine(epoch_warmup={warmup_epochs}, min_lr={min_lr})
train_dir={train_dir}
test_dir={test_dir}
log_dir={log_dir}
val_split=0.2 stratified
normalize=mean[0.5] std[0.5]
""")
        writer.flush()

    if rank == 0:
        print("\n========== Training (No KFold / 1ch) ==========", flush=True)
    ...
    if rank == 0:
        ...
        print("=== Class count (split) ===", flush=True)
    ...
    for epoch in epoch_iter:
        ...
        for step_idx, (inputs, labels) in enumerate(train_loader):
            ...
            if not torch.isfinite(loss):
                if rank == 0:
                    print(f"[WARN] Non-finite loss detected (epoch={epoch}, step={step_idx}). Skipping.", flush=True)
                optimizer.zero_grad(set_to_none=True)
                continue
        ...
        # rank0：val全件で確率集計 → best threshold
        if rank == 0:
            model.eval()
            all_labels_val, all_probs_val = [], []
            nan_batches = 0
            total_batches = 0
            ...
            with torch.no_grad():
                for inputs, labels in eval_val_loader:
                    ...
                    all_probs_val.extend(probs.detach().cpu().numpy().flatten())
                    all_labels_val.extend(labels.detach().cpu().numpy().flatten())

            y_true = np.array(all_labels_val, dtype=np.int32)
            y_prob = np.array(all_probs_val, dtype=np.float32)
            ...
            if nan_batches > 0:
                print(f"[WARN] NaN/Inf probs detected in {nan_batches}/{total_batches} eval batches (epoch={epoch})", flush=True)

            if writer is not None:
                writer.add_scalars("Split/Loss", {"train": epoch_loss, "val": val_epoch_loss}, epoch)
                writer.add_scalars("Split/Accuracy@0.5", {"train": epoch_acc_05, "val": val_epoch_acc_05}, epoch)
                writer.add_scalar("Split/VAL_BestThreshold(Acc)", float(epoch_best_t), epoch)
                writer.add_scalar("Split/VAL_Acc@best_t", float(current_bestt_acc), epoch)
                writer.add_scalar("Split/VAL_AUC(ref)",
                                  float(epoch_metrics["auc"]) if not np.isnan(epoch_metrics["auc"]) else 0.0,
                                  epoch)
                writer.add_scalar("LR_group0(backbone)", optimizer.param_groups[0]["lr"], epoch)
                writer.add_scalar("LR_group1(head)", optimizer.param_groups[1]["lr"], epoch)

                roc_fig = plot_roc_curve_fig(y_true_s, y_prob_s, title=f"ROC (epoch={epoch})")
                if roc_fig is not None:
                    writer.add_image("Split/ROC_curve", fig_to_tensor(roc_fig), global_step=epoch)
                else:
                    writer.add_text("Split/ROC_curve", "Only one class in y_true; ROC unavailable.", epoch)
                ...
                cm_fig = plot_confusion_matrix_fig(
                    cm,
                    class_names=("0", "1"),
                    title=f"CM (t={epoch_best_t:.3f}) epoch={epoch}"
                )
                writer.add_image("Split/Confusion_matrix", fig_to_tensor(cm_fig), global_step=epoch)
                writer.flush()
    ...
    # =====================================================
    # rank0：bestモデルで test 推論 → 提出CSV
    # =====================================================
    if rank == 0:
        if best_state_dict is None:
            raise RuntimeError("best_state_dict が None です。学習が正常に完了していない可能性があります。")
        ...
        torch.save(best_state_dict, os.path.join(weight_dir, "best_no_kfold_1ch.pth"))

        print("\n=== Split Summary ===")
        print("best_val_acc@best_t:", float(best_val_acc))
        ...
        infer_model.load_state_dict(best_state_dict, strict=True)
        infer_model.eval()
        ...
        preds = (all_test_probs >= float(best_threshold)).astype(int)
        ...
        submit_file_path = os.path.join(prediction_dir, "sample_submit_kazma_1222_GPU2_2.csv")
