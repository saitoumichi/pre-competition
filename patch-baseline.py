...
class BreastCancerDataset(Dataset):
    def __init__(self, root_dir, classes, transform=None, patch_size=PATCH, num_patches=4):
    ...
    def _random_crop(self, img: Image.Image):
        ...
        if W < P or H < P:
            img = img.resize((max(W, P), max(H, P)), resample=Image.BILINEAR)
            W, H = img.size
...
# ===========================
    class PatchMIL(nn.Module):
        def __init__(self, backbone_name="tf_efficientnet_b3_ns", pooling="max"):
            super().__init__()
            assert pooling in {"max", "mean", "attn"}
            self.pooling = pooling

            self.backbone = timm.create_model(
                backbone_name,
                pretrained=True,
                num_classes=0,     # features
                global_pool="avg"
            )
            feat_dim = self.backbone.num_features

            # Patch-level classifier head
            self.head = nn.Linear(feat_dim, 1)  # binary logit

            # Attention MIL (gated attention) modules
            # Reference-style: a_k = softmax( w^T (tanh(Vh_k) * sigmoid(Uh_k)) )
            attn_dim = 256
        ...
        def forward(self, x, return_attn: bool = False):
            ...
            feat = self.backbone(x)                 # (B*K, D)
            feat = feat.view(B, K, -1)              # (B, K, D)

            # Patch logits (useful for max/mean pooling)
            logit_patch = self.head(feat)           # (B, K, 1)

            if self.pooling == "max":
                logit_img, _ = torch.max(logit_patch, dim=1)  # (B, 1)
                if return_attn:
                    # For max pooling, attention isn't defined; return a one-hot approx.
                    idx = torch.argmax(logit_patch.squeeze(-1), dim=1)  # (B,)
                    attn = torch.zeros(B, K, device=feat.device)
                    attn[torch.arange(B, device=feat.device), idx] = 1.0
                    return logit_img, attn
                return logit_img

            if self.pooling == "mean":
                logit_img = torch.mean(logit_patch, dim=1)  # (B, 1)
                if return_attn:
                    attn = torch.full((B, K), 1.0 / K, device=feat.device)
                    return logit_img, attn
                return logit_img

            # Attention MIL pooling
            Vh = torch.tanh(self.attn_V(feat))      # (B, K, A)
            Uh = torch.sigmoid(self.attn_U(feat))   # (B, K, A)
            a = self.attn_w(Vh * Uh).squeeze(-1)    # (B, K)
            attn = torch.softmax(a, dim=1)          # (B, K)

            bag_feat = torch.sum(feat * attn.unsqueeze(-1), dim=1)  # (B, D)
            logit_img = self.head(bag_feat)                         # (B, 1)
...
# モデル作成
    model = PatchMIL("tf_efficientnet_b3_ns", pooling="attn")
...
# ===========================
    ...
    class TestDataset(Dataset):
        def __init__(self, root_dir, transform=None, patch_size=224, num_patches=4):
            self.root_dir = root_dir
            self.transform = transform
            self.patch_size = patch_size
            self.num_patches = num_patches
        ...
        def __getitem__(self, idx):
            ...
            patches = []
            for _ in range(self.num_patches):
                p = self._random_crop(img).convert('RGB')
                if self.transform:
                    p = self.transform(p)
                else:
                    p = transforms.ToTensor()(p)
    ...
    test_dataset = TestDataset(test_dir, val_test_transforms, patch_size=224, num_patches=4)
    ...
    with torch.no_grad():
        for patches, filenames in tqdm(test_loader, desc="Test Prediction"):
            patches = patches.to(device, non_blocking=True)
            outputs = model(patches).view(-1)
            probs = torch.sigmoid(outputs)
